...
# --- Load NLP Model ---
# (Relies on requirements.txt to have the .whl file)
try:
...
st.markdown("""
    ...
    p, label, .stMarkdown { color: #cfcfcf; font-weight: 300; }
    .stButton>button {
        width: 100%;
        border-radius: 50px;
...
# --- SIMULATION ENGINE (New Feature) ---
def inject_demo_data(rule_id, rule_url):
    """
    Creates a fake 'past' version of the rule by removing the last paragraph.
    This ensures that when the user runs a Live Audit, they see the missing paragraph as a 'New Addition'.
    """
    real_text = download_rule(rule_url)
    if not real_text: return False
    
    # Create a 'truncated' version (simulating the past)
    lines = real_text.split('\n')
    # Keep top 80% of lines to ensure we "miss" something at the end
    cutoff = int(len(lines) * 0.8) 
    fake_old_text = "\n".join(lines[:cutoff])
    
    # Insert directly into DB with a backdated timestamp
    conn = sqlite3.connect('data/regulations.db')
    c = conn.cursor()
...
with tab2:
    ...
    # Filter for valid comparisons (need at least 2 versions to compare)
    if len(history_df) < 2:
        ...
        with col_a:
            # Default to the older version (index 1)
            ver_a_label = st.selectbox("Baseline Version", version_options, index=1)
        with col_b:
            # Default to the newest version (index 0)
